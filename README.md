...
1. 下载本项目代码并解压到本地
2. 双击项目根目录中的 **`start.bat`（一键启动）**
3. 首次运行会自动安装依赖；服务就绪后，浏览器会自动打开 **http://localhost:3000**

启动窗口需要保持打开。想停止服务时，在窗口中按 `Ctrl+C`，或直接关闭窗口即可。

> 如果提示未检测到 Node.js，请先从 [nodejs.org](https://nodejs.org) 安装 Node.js 18 或更高版本，然后重新双击启动脚本。

## 手动安装与启动

需要排查问题或进行开发时，可以手动启动：

1. 在项目文件夹里打开终端（在文件夹地址栏输入 `cmd` 回车即可）
2. 依次执行：
